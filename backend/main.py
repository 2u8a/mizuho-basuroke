...
def _is_trip_finished(trip_id, now_dt, current_distance_m=None, nearest_stop_id=None):
    """
    便終了判定。

    True を返す条件:
      1. 従来の時間タイムアウト
         now_sec > 終点到着予定時刻 + LOCKED_END_GRACE_SEC
      2. 空間ベースの早期終了
         現在位置が shape終点から50m以内 かつ 現在最寄り停留所が終点停留所
    """
    prog = TRIP_PROGRESS.get(trip_id, [])
    if not prog:
        return False
    ...
    end_stop_id = end_point.get("stop_id")

    now_sec = _seconds_since_midnight(now_dt)
    if now_sec > end_time_sec + LOCKED_END_GRACE_SEC:
        return True

    if current_distance_m is not None and nearest_stop_id:
        remaining_to_end_m = abs(float(current_distance_m) - end_distance_m)
        if remaining_to_end_m <= 50.0 and nearest_stop_id == end_stop_id:
            return True
    ...
    if lock["lock_state"] == "TRIP_LOCKED":
        ...
        if not trip_id:
            ...
            if _is_trip_finished(
                trip_id,
                now_dt,
                current_distance_m=trip_match.get("current_distance_m") if trip_match else None,
                nearest_stop_id=trip_match.get("nearest_stop_id") if trip_match else None,
            ):
